                            ...
                            int fresh_reading, int dt_s)
{
     ...
     *   - While heating, on the first tick after a (re)set we capture
     *     win_start_temp; every heating tick accumulates win_elapsed_s += dt_s.
    ...
    if (duty_pct > 50.0f) {
        if (s->win_elapsed_s == 0)
            s->win_start_temp = temp_c;   /* window opens here */
